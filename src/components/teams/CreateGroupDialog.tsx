...
import { available as executionAvailable, candidateKey, groupTeamCandidates } from '../../core/teams/grouping.js';
...
export type CreateGroupDialogProps = { open: boolean; candidates: TeamMemberCandidate[]; serverAuthority?: boolean; loading?: boolean; error?: string; onRefresh?: () => void; hasMore?: boolean; onLoadMore?: () => void; onClose: () => void; onCreate: (input: GroupCreateInput) => Promise<unknown> };
/** Selection belongs to the draft, not to a refreshed directory row. */
export function CreateGroupDialog({ open, candidates, serverAuthority, loading, error: directoryError, onRefresh, hasMore, onLoadMore, onClose, onCreate }: CreateGroupDialogProps) {
  ...
  const needsPreflight = (candidate: TeamMemberCandidate) => Boolean(serverAuthority && candidate.binding.availability?.state === 'unchecked');
  // Unchecked directory entries are selectable drafts, not verified capabilities.
  // Only Server preflight can authorize execution or confirm a Leader/standby.
  const available = (candidate: TeamMemberCandidate) => executionAvailable(candidate) || needsPreflight(candidate);
  const maySelectLeader = (candidate: TeamMemberCandidate) => available(candidate) && (candidate.binding.capabilities.leader || needsPreflight(candidate));
  ...
  const leaderCandidates = members.filter(maySelectLeader);
  ...
  const standbyCandidates = candidates.filter(candidate => candidate.binding.kind === 'cloud' && maySelectLeader(candidate));
  const actualStandby = standbyCandidates.some(candidate => candidate.binding.bindingRef === standby) ? standby : '';
  const groups = groupTeamCandidates(candidates).map(versions => versions.sort((a, b) => Number(available(b)) - Number(available(a))));
  ...
  async function create() {
    ...
    setError('');
    const input = { name: name.trim(), members: members.map(member => ({ memberId: member.memberId, name: member.name, bindingRef: member.binding.bindingRef, ...(member.responsibility?.trim() ? { responsibility: member.responsibility.trim() } : {}) })), leaderMemberId: actualLeader, ...(serverAuthority && localLeader && actualStandby ? { leaderStandbyBindingRef: actualStandby } : {}) };
    try {
  ...
  return <dialog ref={dialog} className="ksadk-teams team-create-dialog" aria-labelledby={headingId} onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}><form onSubmit={event => { event.preventDefault(); void create(); }}>
    ...
    <fieldset disabled={busy}><legend>选择成员 <span>{selected.length} / 8</span></legend><div className="team-directory-search"><input aria-label="搜索 Agent" value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索名称或职责" />{onRefresh && <button type="button" className="team-text-button" disabled={loading} onClick={onRefresh}>刷新</button>}</div>
      ...
      {!loading && !candidates.length ? <p className="team-muted">还没有 Agent。先创建 Agent 并完成构建，再回来组队。</p> : <div className="team-candidate-list">{visible.map(versions => {
        ...
        const description = needsPreflight(candidate) ? '创建时验证执行能力' : candidate.description || candidate.binding.description || (candidate.binding.capabilities.leader ? '可担任 Leader' : '任务成员');
      ...
      {hasMore && onLoadMore && <button type="button" className="team-button" disabled={loading || busy} onClick={onLoadMore}>加载更多成员</button>}
    ...
    <label className="team-field">指定 Leader<select value={actualLeader} onChange={event => setLeader(event.target.value)} disabled={busy || !leaderCandidates.length}><option value="" disabled>从已选成员中指定</option>{leaderCandidates.map(candidate => <option key={candidate.memberId} value={candidate.memberId}>{candidate.name}{needsPreflight(candidate) ? ' · 创建时验证' : ''}</option>)}</select><small>Leader 审核成员成果，你处理工具审批和最终验收。{leaderCandidates.some(needsPreflight) && ' 待检查候选需通过服务端能力验证后才能担任 Leader。'}</small></label>
    {serverAuthority && localLeader && <details className="team-version-choice"><summary>本地离线时继续协作</summary><label className="team-field">Leader 云端备用<select value={actualStandby} onChange={event => setStandby(event.target.value)} disabled={busy || loading}><option value="">暂不配置</option>{standbyCandidates.map(candidate => <option key={candidate.binding.bindingRef} value={candidate.binding.bindingRef}>{candidate.name} · {needsPreflight(candidate) ? '创建时验证' : candidate.binding.version || candidate.binding.buildId || '固定版本'}</option>)}</select></label><p className="team-muted">服务端验证同一发布及实际执行能力后，备用才可启用。云端模型、工具和任务资料需预先准备；本地凭据不会复制到云端。</p>{!standbyCandidates.length && <p className="team-muted">还没有可用的云端 Leader 候选。可以先创建团队，接入节点后再配置。</p>}</details>}
