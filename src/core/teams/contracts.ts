...
export const groupSchema = z.object({ groupId: id, name: z.string().min(1), authorityRef: id, tenantId: id, ownerSubject: id, policy: z.object({ taskAcceptance: z.enum(['leader', 'human', 'result']), peerWake: z.boolean() }).optional(), leaderMemberId: id, revision, status: z.enum(['active', 'archived']), createdAt: timestamp, updatedAt: timestamp }).passthrough();
export const memberSchema = z.object({ memberId: id, groupId: id, name: z.string().min(1), role: z.enum(['leader', 'member']), responsibility: z.string().max(2000).optional(), bindingRef: id, binding: z.object({ bindingRef: id, providerRef: id, kind: z.enum(['local_build', 'a2a', 'cloud']), agentId: id, capabilities }).passthrough(), sessionId: id, revision, status: z.enum(['active', 'removed', 'unavailable']), executionStatus: z.enum(['idle', 'queued', 'running', 'waiting', 'needs_attention', 'unavailable']) }).passthrough();
export const runMemberSchema = memberSchema.extend({ runMemberId: id, teamRunId: id, groupRevision: revision });
...
const workspaceSchema = z.object({ sourcePath: z.string().min(1), baseRef: z.string().optional(), inputs: z.array(z.string()).optional(), mode: z.enum(['auto', 'directory']).optional() }).passthrough();
const standbySchema = z.object({ groupId: id, teamRunId: id, state: z.enum(['armed', 'blocked', 'active']), primaryNodeId: id, primarySessionId: id, primaryBindingRef: id, standbyNodeId: id, standbyBindingRef: id, buildDigest: id, epoch: revision, reason: z.string().nullable().optional(), automaticFailback: z.literal(false), sessionId: id.optional(), checkpointId: id.optional() }).passthrough();
export const teamRunSchema = z.object({ teamRunId: id, groupId: id, groupRevision: revision, policy: groupSchema.shape.policy, revision, goalMessageId: id, goal: z.string(), result: z.string().optional(), resultHistory: z.array(z.object({ result: z.string(), reason: z.string(), revision, requestedAt: timestamp }).passthrough()).optional(), workspace: workspaceSchema.nullable().optional(), leaderStandby: standbySchema.nullable().optional(), leaderMemberId: id, status, dispatchSuspended: z.boolean(), dispatchEpoch: revision, budget: z.object({ maxMembers: revision, maxConcurrent: revision, maxStarts: revision, startsUsed: revision, maxHops: revision }).passthrough(), createdAt: timestamp, updatedAt: timestamp }).passthrough();
...
export const groupSnapshotSchema = z.object({ apiVersion: z.literal(TEAMS_API_VERSION), group: groupSchema, watermark: revision, members: z.array(memberSchema), runMembers: z.array(runMemberSchema).optional(), messages: z.array(messageSchema), tasks: z.array(taskSchema), teamRuns: z.array(teamRunSchema), deliveries: z.array(deliverySchema), interactions: z.array(interactionSchema), artifacts: z.array(artifactSchema).optional() }).passthrough();
...
export function decodeGroupSnapshot(raw: unknown): GroupSnapshot {
  ...
  const collections = [snapshot.runMembers || [], snapshot.members, snapshot.messages, snapshot.tasks, snapshot.teamRuns, snapshot.deliveries];
